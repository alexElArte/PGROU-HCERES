...
function PublicationList(props) {
    ...
    const [targetPublication, setTargetPublication] = React.useState(false);
    const [showPublicationAdd, setShowPublicationAdd] = React.useState(false);
    const [showPublicationDelete, setShowPublicationDelete] = React.useState(false);
    const [listChangeCount, setListChangeCount] = React.useState(0);

    const handleHideModal = (msg = null) => {
        setShowPublicationAdd(false);
        setShowPublicationDelete(false);
        if (msg) {
    ...
    React.useEffect(() => {
        if (!targetResearcher) {
            ...
            fetchResearcherActivities(targetResearcher.researcherId)
                .then(list => {
                    setPublicationList(list.filter(a => a.idTypeActivity === ActivityTypes.PUBLICATION));
                });
        }
    ...
    if (!publicationList) {
        ...
        if (publicationList.length === 0) {
            return <div className={"row"}>
                <br />
                <div className={"col-8"}>
                    <h3>Aucune publication n'est enregistré</h3>
                </div>
                <div className={"col-4"}>
                    {showPublicationAdd &&
                        <PublicationAdd targetResearcher={targetResearcher} onHideAction={handleHideModal} />}
                    <button className="btn btn-primary" data-bs-toggle="button"
                        onClick={() => setShowPublicationAdd(true)}>
                        <AiOutlinePlusCircle /> &nbsp; Ajouter une publication
                    </button>
                </div>
            </div>;
        }
                    ...
                    <div>
                        <OverlayTrigger placement="bottom" delay={{ show: 250, hide: 400 }} overlay={deleteTooltip}>
                            <button className="btn btn-outline-danger btn-sm" onClick={() => {
                                setTargetPublication(row);
                                setShowPublicationDelete(true);
                            }}>
                                <AiFillDelete />
                            </button>
                        </OverlayTrigger>
        ...
        if (!targetResearcher) {
            ...
            title = "Liste des publications pour les Chercheurs";
        }
                        ...
                        props => (
                            <div>
                                ...
                                <div className={"row"}>
                                    ...
                                    <div className={"col-4"}>
                                        {showPublicationAdd &&
                                            <PublicationAdd targetResearcher={targetResearcher}
                                                onHideAction={handleHideModal} />}
                                        {showPublicationDelete &&
                                            <PublicationDelete targetPublication={targetPublication}
                                                onHideAction={handleHideModal} />}
                                    </div>
                                </div>
                                <div className="row">
                                ...
                                <BootstrapTable
                                    ...
                                    pagination={paginationFactory(paginationOptions(publicationList.length))}
                                    striped
                                    hover
                                    condensed
                                    selectRow={selectRow}
